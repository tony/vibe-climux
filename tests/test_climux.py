...
class TestInputOutput:
    ...
    async def test_send_input_to_process(
        ...
        # Send input
        send_result = await climux_client.request(
            "send",
            {
                "id": process_id,
                "data": "Hello, World!",
...
class TestLogging:
    ...
    async def test_get_logs_with_limit(
        ...
        # Get limited logs
        logs = await climux_client.request(
            "logs",
            {
                "id": process_id,
                "lines": 5,
        ...
        assert len(logs) == 5
        # Should get the last 5 lines - check for expected content
        log_contents = [log["content"] for log in logs]
        # The last lines should include either "Done counting" or system messages
        assert any(
            "Done counting" in content
            or "Count:" in content
            or "exit" in content.lower()
    ...
    async def test_snapshot_custom_lines(
        ...
        snapshot = await climux_client.request(
            "snapshot",
            {
                "id": process_id,
                "lines": 10,
...
class TestCleanup:
    ...
    async def test_pid_journal_cleanup(
        ...
        journal_data = json.loads(pid_journal.read_text())
        assert (
            str(proc["id"]) not in journal_data or journal_data[str(proc["id"])] is None
        )
...
class TestStress:
    ...
    async def test_rapid_restart(
        ...
        # Start process
        proc = await climux_client.request(
            "start",
            {
